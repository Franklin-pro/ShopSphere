    ...
    <div class="flex bg-white rounded-lg shadow-lg overflow-hidden mx-auto max-w-sm lg:max-w-4xl">
        ...
        <div class="w-full p-8 lg:w-1/2">
            ...
            <div class="mt-4">
                ...
                <input v-model="phoneNumber" @input="formatPhoneNumber" class="bg-gray-200 text-gray-700 focus:outline-none focus:shadow-outline border border-gray-300 rounded py-2 px-4 block w-full appearance-none" type="text" />
            </div>
            <div class="mt-4">
                <div class="flex justify-between">
            ...
            <div class="mt-8">
                <button class="bg-[#DB4444] text-white font-bold py-2 px-4 w-full rounded hover:bg-gray-600">Login</button>
            </div>
            <div class="mt-4 flex items-center justify-between">
                <span class="border-b w-1/5 md:w-1/4"></span>
                <a href="/auth/login_account" class="text-xs text-gray-500 uppercase">or sign In</a>
                <span class="border-b w-1/5 md:w-1/4"></span>
            </div>
        </div>
    </div>
...
const formatPhoneNumber = () => {
  ...
  // Apply formatting (e.g., (123) 456-7890 for US numbers)
  if (cleaned.length <= 3) {
    phoneNumber.value = cleaned;
  } else if (cleaned.length <= 6) {
    phoneNumber.value = `(${cleaned.slice(0, 3)}) ${cleaned.slice(3)}`;
  } else {
    phoneNumber.value = `(${cleaned.slice(0, 3)}) ${cleaned.slice(3, 6)}-${cleaned.slice(6, 12)}`;
  }
};
</script>
